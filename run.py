#!/usr/bin/env python3
"""Launch PRIDE as ONE backend (Discord bot + FastAPI web + misinfo), sharing a
SINGLE vLLM model on the GPU. This is the main entry point.

Running the web/Discord stack and src/misinfo/serve.py separately would each load
their own vLLM engine (two ~4B models on two GPUs). This entry point loads the model
ONCE (via ModelServices) and makes the misinfo GraphCheck verifier reuse that same
engine, so the whole system fits one GPU:
...
ML cache/scratch location is configurable via env (or .env): PRIDE_CACHE_DIR
(default /workspace/.cache); the OS TMPDIR is derived from it. (This is separate
from PRIDE_TMP_DIR / paths.tmp_dir in config.yaml, which is the app's downloaded-
media directory.)
"""
...
# Load .env early so it can configure the cache/tmp locations below (this block
# must run before any torch/vllm import, hence before the rest of the program).
load_dotenv()

# Base directory for all ML caches + the OS scratch dir (TMPDIR). One knob:
# set PRIDE_CACHE_DIR to relocate everything (e.g. onto a bigger volume). The
# default matches the previous hardcoded location so existing setups are
# unaffected. NOTE: this is NOT config.yaml's paths.tmp_dir (the app's media
# download dir, overridable via PRIDE_TMP_DIR) — different concept, distinct knob.
_CACHE_DIR = os.environ.get("PRIDE_CACHE_DIR", "/workspace/.cache")
_TMP_DIR   = os.path.join(_CACHE_DIR, "tmp")

# Redirect all ML caches before any torch/vllm imports. `setdefault` so an
# explicitly-exported HF_HOME/TORCH_HOME/etc. still overrides our derived value.
for _var, _default in [
    ("HF_HOME",                 os.path.join(_CACHE_DIR, "huggingface")),
    ("HUGGINGFACE_HUB_CACHE",   os.path.join(_CACHE_DIR, "huggingface", "hub")),
    ("TORCH_HOME",              os.path.join(_CACHE_DIR, "torch")),
    ("TORCHINDUCTOR_CACHE_DIR", os.path.join(_CACHE_DIR, "torch", "inductor")),
    ("TRITON_CACHE_DIR",        os.path.join(_CACHE_DIR, "triton")),
    ("VLLM_CACHE_ROOT",         os.path.join(_CACHE_DIR, "vllm")),
    ("TMPDIR",                  _TMP_DIR),
]:
