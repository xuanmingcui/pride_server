...
class VLLMBackend(BaseMLLM):
    ...
    def _build_request(
        self, prompt_text: str, frames: Optional[List[Image.Image]], fps: Optional[float]
    ) -> Dict[str, Any]:
    ...
    def generate_text(self, prompt: str) -> str:
        ...
        outputs = self.llm.generate([{"prompt": prompt}], sampling_params=sampling)
    ...
    def generate_text_batch(self, prompts: List[str]) -> List[str]:
        if not prompts:
            return []
        sampling = self.SamplingParams(max_tokens=512, temperature=0.0)
        outputs = self.llm.generate([{"prompt": p} for p in prompts], sampling_params=sampling)
    ...
    def generate_raw(
        ...
        req = self._build_request(prompt, frames, fps)
...
class TransformersVLBackend(BaseMLLM):
    ...
    def generate_text(self, prompt: str) -> str:
        return self._run_one(prompt, None, None)
    ...
    def generate_raw(
        ...
        try:
            return self._run_one(prompt, frames, fps)
        finally:
